...
$error_title = esc_html__( '404', 'mireya' );
$title = get_field( 'p404_title', 'option' );
if ( ! $title ) {
	$title = esc_html__( 'Page not found...', 'mireya' );
}
$content = get_field( 'p404_content', 'option' );
if ( ! $content ) {
	$content = '<p>' . esc_html__( 'We are unable to find the page you are looking for.', 'mireya' ) . '</p>';
}
